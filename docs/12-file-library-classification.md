...
>
>    *Clarification (2026-08-13):* the assistant's `library_search` tool reads `library_files` under the caller's own RLS and returns **metadata and AI-generated derivatives only** — display name, folder, category, the stored `ai_summary`/`ai_key_figures` — never file contents, `storage_path` or `sha256`. A summary is the *output* of a crossing this document already authorises, so replaying it to the same provider class creates no new egress; the projection allowlist (`PROJECTIONS.library_search`) pins exactly these fields and the global blocklist still strips `file_name`/`storage_path`/`sha256` everywhere. The two-channel rule for *contents* stands.
...
**Supported formats.** PDF (text layer), Word `.doc`/`.docx`, Excel `.xls`/`.xlsx`/`.xlsm`/`.xlsb`/`.ods`, PowerPoint `.pptx`, CSV, plain text and JSON go down the text branch; images go to the vision model.
...
Legacy binary Word `.doc` is read by `word-extractor`; `.pptx` is unzipped with `fflate` (8 KB, zero-dependency) and its `<a:t>` text runs are read slide by slide, **including speaker notes** — the argument behind a slide is often where the substance lives.

| Format | Parser |
|---|---|
| `.pdf` | `unpdf` (text layer; falls back to `no_text_layer` when scanned) |
| `.docx` | `mammoth` |
| `.doc` | `word-extractor` (OLE compound) |
| `.xls` `.xlsx` `.xlsm` `.xlsb` `.ods` | SheetJS (vendor CDN build) |
| `.pptx` | `fflate` unzip + `<a:t>` runs, slides in order + notes |
| `.csv` `.txt` `.json` | UTF-8 passthrough |
| images | vision model |
