 ...
 */
export async function enqueueApproval(args: EnqueueArgs): Promise<string> {
  ...
  if (error) {
    ...
    // is true deduplication; if it reached a terminal state the caller means a
    // genuinely new request, so re-insert under a disambiguated key.
    if (error.code === "23505") {
      const { data: existing } = await db
        ...
        .maybeSingle();
      if (existing?.state === "pending") return existing.id as string;
